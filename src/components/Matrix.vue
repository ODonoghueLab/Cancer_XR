...
export default {
  ...
  methods : {
    ...
    redirect: function (redirectLink) {
      ...
      query = query.replace(/^&/, "?");
      return 'https://test.aquaria.app/' + redirectLink.Organism + '/' + redirectLink.Name + ((redirectLink.PDB == null) ? '' : '/' + redirectLink.PDB) + ((redirectLink.Chain == null) ? '' : '/' + redirectLink.Chain) + query + ((redirectLink.Orientation == null) ? '' : '#?' + redirectLink.Orientation)
